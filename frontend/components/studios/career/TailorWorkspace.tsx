...
export function TailorWorkspace({ initialJobDescription = "", initialResumeId = "", resumes }: { initialJobDescription?: string; initialResumeId?: string; resumes: ResumeFileInfo[] }): JSX.Element {
  ...
  return <div className="grid gap-6 xl:grid-cols-[minmax(0,.9fr)_minmax(0,1.1fr)]">
    <section className="rounded-xl border border-border bg-workspace-raised p-5 sm:p-6">
      ...
      <div className="mt-5"><ResumeIntake file={file} onFileChange={(next) => { setFile(next); if (next) setResumeId(""); resetReview(); }} onStructuredImport={async (payload) => { setBusy("review"); setError(null); try { const ingested = await careerStudioClient.createSource(payload); setFile(null); setResumeId(""); setSourceId(ingested.source.id); setClaims(ingested.claims); setResult(null); } catch (reason) { setError(reason instanceof Error ? reason.message : "Structured import failed"); throw reason; } finally { setBusy(null); } }} /></div>
