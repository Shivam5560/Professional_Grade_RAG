...
function NamedRecords({ label, records, onChange }: { label: string; records: ResumeGenNamedRecord[]; onChange(records: ResumeGenNamedRecord[]): void }) { return <section><div className="flex items-center justify-between"><h3 className="font-semibold">{label}</h3><Button size="sm" variant="outline" onClick={() => onChange([...records, { name: "" }])}><Plus className="mr-2 h-3.5 w-3.5" />Add</Button></div><div className="mt-3 space-y-3">{records.map((record, index) => <div className="grid gap-2 rounded-lg border border-border bg-workspace-inset p-3 sm:grid-cols-[1fr_1fr_8rem_auto]" key={index}><Input placeholder="Name" value={record.name} onChange={(e) => onChange(records.map((item, i) => i === index ? { ...item, name: e.target.value } : item))} /><Input placeholder={label === "Languages" ? "Proficiency" : "Issuer"} value={label === "Languages" ? record.proficiency ?? "" : record.issuer ?? ""} onChange={(e) => onChange(records.map((item, i) => i === index ? { ...item, [label === "Languages" ? "proficiency" : "issuer"]: e.target.value } : item))} /><Input placeholder="Date" value={record.date ?? ""} onChange={(e) => onChange(records.map((item, i) => i === index ? { ...item, date: e.target.value } : item))} />{label === "Certifications" ? <Input className="sm:col-span-3" placeholder="Credential link" value={record.link ?? ""} onChange={(e) => onChange(records.map((item, i) => i === index ? { ...item, link: e.target.value } : item))} /> : null}<Button aria-label={`Remove ${label} ${index + 1}`} size="icon" variant="ghost" onClick={() => onChange(records.filter((_, i) => i !== index))}><Trash2 className="h-4 w-4" /></Button></div>)}</div></section>; }
