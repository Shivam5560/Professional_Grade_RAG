  ...
  return (
    <AuraSqlPage title="Schema contexts" description="Keep reusable table selections close to the questions they ground." actions={<Button onClick={() => router.push('/aurasql/contexts/new')}><Plus className="mr-2 h-4 w-4" />New context</Button>}>
      ...
      {loading ? <p className="border-y border-border/60 py-12 text-center text-sm text-muted-foreground">Loading schema contexts…</p> : (
        <div className="divide-y divide-border/60 border-y border-border/60 bg-workspace-raised px-4">
          {contexts.length === 0 ? <div className="py-16 text-center"><p className="text-sm font-medium">No saved schema contexts</p><p className="mt-1 text-xs text-muted-foreground">Choose a connection and group the tables that belong to your next question.</p><Button className="mt-4" onClick={() => router.push('/aurasql/contexts/new')}>Create context</Button></div> : contexts.map((context) => (
